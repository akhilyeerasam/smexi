...
sub handleOneProject {
    ...
    my $muncipalityId = $elementToAdd->{properties}->{municipality_id};
    $elasticsearchStructure->{name} = $muncipalityId;
    $elasticsearchStructure->{municipality_id} = $elasticsearchStructure->{name};  # municipalityId == name

    $muncipalityId =~ s/^0*//;  # remove trailing zeroes
    # if ( exists( $metadata->{$muncipalityId} ) ) {
    ...
    #     #     'Ortsbezirke Nr' => '17'
    #     my $metadataForOneElement = $metadata->{$muncipalityId};
    ...
    # }
    # else {
    #     #$elasticsearchStructure->{description} = "Unknown statistical region $muncipalityId";
    #     die "No metadata found for $muncipalityId\n";
    # }
    ...
    if ( 1 ) {  # currently no region contains illegal data after repairing via prepair
        ...
        $elasticsearchStructure->{STAT_BEZ_Ids} = $elementToAdd->{properties}->{STAT_BEZ_Ids};
        $elasticsearchStructure->{municipality_name} = $elementToAdd->{properties}->{PGIS_TXT};
